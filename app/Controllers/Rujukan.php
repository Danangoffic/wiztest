...
class Rujukan extends ResourceController 
{
    ...
    public function save_rujukan()
    {
        ...
        $id_rujukan     =   $this->request->getPost('id_rujukan');
        ...
        $id_cust_rujukan = $this->rujukan_model->getInsertID();

        $afiliasiController = new Afiliasi;
        $invoice_corporate = $afiliasiController->afiliasi_invoice($id_cust_rujukan, "rujukan");
                    $arr_invoice_corporate = ['invoice_number' => $invoice_corporate];
                    $this->customer_rujukan_model->update($id_cust_rujukan, $arr_invoice_corporate);
