                    ...
                    <div class="card card-primary">
                        ...
                        <div class="card-footer">
                            <div class="float-right">
                                ...
                                <button class="btn btn-success" id="saveButton" type="button" onclick="saveButton()"><i class="fa fa-check"></i> Validasi</button>
                            </div>
                        </div>
...
<script>
    ...
    function load_data_hasil_customer() {
        let url = "<?= base_url('api/hasil/by_customer/') . $id_customer; ?>";
        $.get(url, function(data) {
            let status = data.status;
            if (status == "success") {
                let status_antigen = data.status_antigen;
                let status_pemeriksaan = data.status_pemeriksaan;
                let status_kirim = data.status_kirim;
    ...
    function saveButton() {
        ...
        let waktu_ambil_sampling = $("#waktu_ambil_sampling").val();
        let waktu_periksa_sampling = $("#waktu_periksa_sampling").val();
        let waktu_selesai_periksa = $("#waktu_selesai_periksa").val();
        let status_antigen = $("#status_antigen").val();
        let status_pemeriksaan = $("#status_pemeriksaan").val();
        let status_kirim = $("#status_kirim").val();
        let id_hasil = $("#id").val();
        let data_send = {
            status_antigen,
            status_pemeriksaan,
        ...
        let url_post = "<?= base_url("api/save/verifikasi"); ?>";
        $.post(url_post, data_send, function(data, status) {
            let dataStatus = data.status;
            if (status == "success" && dataStatus == "success") {
                document.getElementById("saveButton").disabled = false;
                $("#saveButton").html("Loading...");
                load_data_hasil_customer();
            } else {
                alert("Gagal simpan");
            }
        })
